...
function spawnrandombloon()
{
	...
	if (!has(bloonData.atrs, atr.moab) || hasany(bloonData.atrs, atr.bob | atr.honey))
	{
		...
		if (random(1) < clamp((currentRound - (o >= 5 ? (o >= 10 ? 30 : 15) : 0)) / 30 * 0.5, 0, 0.5)) mods |= atr.regrow;
		if (random(1) < clamp((currentRound / (bloonData.hp > 1 ? 2 : 1) - 20) / 40 * 0.5, 0, 0.5)) mods |= atr.latex;
	}
